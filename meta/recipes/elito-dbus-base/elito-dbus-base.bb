...
do_install() {
	mkdir -p ${D}/root ${D}/var/run/dbus-root ${D}/etc/tmpfiles.d
	ln -s /var/run/dbus-root ${D}/root/.dbus
	install -p -m 0644 ${WORKDIR}/01-dbus-root.conf ${D}/etc/tmpfiles.d/
}

PACKAGES = "${PN}"
FILES_${PN} = "/root/.dbus /etc/tmpfiles.d/01-dbus-root.conf"
